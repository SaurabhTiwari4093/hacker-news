...
export default function NewsComp(props) {
    ...
    const updateNews = async () => {
        ...
        const url = `https://hn.algolia.com/api/v1/search_by_date?page=${page}&hitsPerPage=${pageSize}`;
    ...
    const Next = async () => {
        const url = `https://hn.algolia.com/api/v1/search_by_date?page=${page + 1}&hitsPerPage=${pageSize}`;
        setPage(page + 1)
        let data = await fetch(url);
        let parsedData = await data.json()
    ...
    return (
        <>
            <h1 className='text-center' style={{ marginTop: '100px', textShadow: "1px 1px 1px #2222226b" }}>Top Latest Headlines </h1>
            <InfiniteScroll
